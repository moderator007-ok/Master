...
def generate_thumbnail(
    video_file, thumb_output="generated_thumb.jpg", time_stamp="00:00:01"
):
    cmd = [
        "ffmpeg",
        "-y",
...
async def upload_handler(event):
    async with bot.conversation(event.chat_id) as conv:
        try:
            ...
            q5 = await conv.send_message(
                "Enter resolution (choose: 144, 240, 360, 480, 720, 1080):"
            )
            ...
            for i in range(start_index - 1, len(links)):
                ...
                if "visionias" in url:
                    ...
                    url = requests.get(
                        api_url, headers={"x-access-token": "TOKEN"}
                    ).json()["url"]
                ...
                try:
                    cc = (
                        f"**[📽️] Vid_ID:** {str(i+1).zfill(3)}. {name1}{caption}.mkv\n"
                        f"**Batch Name »** {batch_name}\n"
                        f"**Downloaded By :** TechMon ❤️‍🔥 @TechMonX"
                    )
                    cc1 = (
                        f"**[📁] Pdf_ID:** {str(i+1).zfill(3)}. {name1}{caption}.pdf\n"
                        f"**Batch Name »** {batch_name}\n"
                        f"**Downloaded By :** TechMon ❤️‍🔥 @TechMonX"
                    )
                    if "drive" in url:
                        ...
                        try:
                            ...
                            await bot.send_file(
                                event.chat_id, file=f"{file_name}.pdf", caption=cc1
                            )
                            counter += 1
                            ...
                            if (percent - last_percent >= 5) or (current == total):
                                ...
                                try:
                                    await bot.edit_message(
                                        event.chat_id, progress_msg.id, text
                                    )
                                except Exception:
                        ...
                        with open(res_file, "rb") as file_obj:
                            uploaded_file = await fast_upload(
                                bot, file_obj, progress_callback=progress_callback
                            )
